...
var plusBottom = 'img/focalDown.png'
var minusBottom = 'img/focalDown.png'
...
//js卡住位置（因为位置原因window.parent.parent.parent.parent都是向父级调用，在此没有文件且位置也不对，但有注释说明这三个没用，所以注释掉了）
...
function GetAreaInfo() {
  ...
  try {
    var url =
      '/CMSJM/public/patrol/ajax/updatearea.php?task=GetAllAreaByStation&StationID=' + StationID

    // var url = '/dalirobotcms/ajax/updatearea.php?task=GetAllAreaByStation&StationID=' + StationID
    //		alert("url="+url);
    xmlHttpGetAreaInfo.onreadystatechange = function () {
      GetAreaInfoRes()
    }
    xmlHttpGetAreaInfo.open('get', url, false)
    //post这一句必须加上
    xmlHttpGetAreaInfo.setRequestHeader(
      'Content-Type',
...
function GetDeviceInfo() {
  ...
  try {
    var url =
      '/CMSJM/public/patrol/ajax/getdeivcedata.php?SearchDeviceType=All&StationID=' + StationID
    //		alert("url="+url);
    xmlHttpAllDev.onreadystatechange = function () {
      GetAllDevRes()
    }
    xmlHttpAllDev.open('get', url, false)
    //post这一句必须加上
    xmlHttpAllDev.setRequestHeader(
      'Content-Type',
      'application/x-www-form-urlencoded;charset=utf-8'
    )
...
function init() {
  ...
  //区域间隔也要筛选
  ele_init('StationID', 3, '变电站选择', {
    OP: window.parent.parent.parent.parent.Station_s,
  ...
  if (DtreeAutoSelectType == 'FromMap') {
    ...
    for (var i = 0; i < window.parent.parent.parent.maxWindow; i++) {
      var iframeUrl =
        window.parent.parent.parent.frames['mdi'].document.frames['mdi_' + i].location.pathname
    ...
    if (MapMainIframeID != '') {
      var AreaHintData =
        window.parent.parent.parent.frames['mdi'].document.frames['mdi_' + MapMainIframeID].document
          .frames['mapifr'].AreaHintData
    ...
    for (var i = 0; i < window.parent.parent.parent.maxWindow; i++) {
      ...
      //alert("iframeUrl="+iframeUrl);
      if (
        iframeUrl.indexOf('firstdevview.html') != -1 ||
        iframeUrl.indexOf('firstdevview.html') != -1
      ) {
        MapMainIframeID = i
        break
    ...
    if (MapMainIframeID != '') {
      var AreaHintData =
        window.parent.parent.parent.frames['mdi'].document.frames['mdi_' + MapMainIframeID].document
          .frames['firstdevmapifr'].AreaHintData
      var m = AreaHintData.length
      //alert("m="+m);
      for (var i = 0; i < m; i++) {
        if (AreaHintData[i].isSelected == 1) {
  ...
  if (window.parent.parent.parent.parent.PageStyle == 0) {
    ...
    $('#StationID_s').css({
      background: 'rgba(44,53,89,1)',
      color: '#a1bec5',
      border: '0px solid #a1bec5',
      width: '220px',
      height: '27px',
      'margin-left': '-5px',
    })
...
/*
var xmlHttpQueryInspectLog="";
function QueryInspectLog(InspectionLog_ID)
{
	if(InspectionLog_ID==""){return false;}

	xmlHttpQueryInspectLog=createXMLHttpRequest();
  try
  {
		var q="task=QueryInspectErrorDeviceID&InspectionLog_ID="+InspectionLog_ID;
		//alert("q="+q);
		var url = "/dalirobotcms/ajax/queryinspectionresult.php";
		xmlHttpQueryInspectLog.onreadystatechange=function(){QueryInspectLogRes();};
		xmlHttpQueryInspectLog.open("post", url, true);
		//post这一句必须加上
		xmlHttpQueryInspectLog.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;charset=utf-8');
		xmlHttpQueryInspectLog.send(q);
  }
  catch(e){alert(e);}
}

function QueryInspectLogRes()
{
  if(xmlHttpQueryInspectLog.readyState == 4)
  {
    if(xmlHttpQueryInspectLog.status == 200)
    {
			var s=xmlHttpQueryInspectLog.responseText;
			//alert(s);
			if(s =="")
			{
				return;
...
function CreateTree() {
  //	alert("ADDTL="+ADDT.length);
  //	alert("FADTL="+FADT.length);
  //	alert("ADDT.len="+ADDT.length);
  SelectDev = []
  SelectDev.length = 0
  d = new dTree('d')
  ...
  ////		if(AllDev.length==0 && n<10){
  //		if(ADDT.length==0 && n<10){
  ////			if(AllDevList==1)
  ////			{
  //				ADDT =new Array();
  ////				AllDev=new Array();
  ...
  //////				AllDev=window.parent.parent.parent.parent.AllDev.slice(0);
  ////			}
  //			FADT=window.parent.parent.parent.parent.FinalArea.slice(0);
  //			n++;
  //		}else{
  ...
  //			var l=ADDT.length;
  //			for (var i = 0; i < l; i++) {
  //				var objtemp = new FinalAreaObj(ADDT[i].Device_Id, ADDT[i].Device_Desc, ADDT[i].FirstDevID, 6, 3);
  //				FADT.push(objtemp);
  //			}
  //			//alert("FADT="+FADT.length);
  //
  //
  //			TreeJSN=arrayToTree(FADT);
  //console.log(TreeJSN);
...
function RightKeyMenuDesktop() {
  if (UseRightKeyMenuFlag == '0' || UseRightKeyMenuFlag == '') {
    return false
  }
      ...
      if (Area_Id != '') {
        ...
        //一次设备
          ...
          if (RightKeyFrom == 'deviceinfomanage') {
            ...
            menu.AddItem(
              'ExportFirstDevInspectionresult',
              'start_update',
              ' ',
              '导出全面巡检结果',
              'rbpm',
...
function OpenVideoByDevice(Device_Id) {
  ...
  if (DeviceType == 2) {
    var DevID = Inspection_Id + '_0'
  } else {
    var DevID = Inspection_Id + '_1'
  }

  //打开视频
  window.parent.frames['videomonitor'].OpenVideoByDevID(DevID, '', 1)

  if (ParentUrl == 'inteligentOperation') {
    return
  }
  //呼叫预置位
...
function PollingAllDev(AreaType, Area_Id) {
  ...
  try {
    ...
    var url =
      '/CMSJM/public/patrol/ajax/pollingAllDev.php?AreaType=' +
      AreaType +
      '&Area_Id=' +
      Area_Id +
      '&RunTime=' +
      RunTime
    //alert("url="+url);
    xmlHttpPollingAllDev.onreadystatechange = function () {
      PollingAllDevRes()
    }
...
function RunInspectionTask(Device_Id) {
  ...
  try {
    var url =
      '/CMSJM/public/patrol/ajax/manualDevice.php?Device_Id=' + DevieIDOnRobot + '&CarID=' + CarID
    //		alert("url="+url);
    xmlHttpInspectPreset.onreadystatechange = function () {
      RunInspectionTaskRes()
    }
    xmlHttpInspectPreset.open('get', url, true)
    //post这一句必须加上
    xmlHttpInspectPreset.setRequestHeader(
      'Content-Type',
      'application/x-www-form-urlencoded;charset=utf-8'
    )
