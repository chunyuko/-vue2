...
// ajax/taskList.php
// task=getDevListTree&stationID=417DA09F-8A2D-83A3-AC4A-AA94CD126222&DeviceType=1&AreaID=1&GridDevType=1&GridDevSel=1&DevSelfType=1&CarType=1&Province=1&Company=1&OperatorID=1
function GetTaskInfo()      //根据stationid获取
{
	StationID=get_ele_value("StationID");
	xmlHttpGetAreaInfo=createXMLHttpRequest();
  try
  {
		var url = "/dalirobotcms/ajax/updatearea.php?task=GetAllAreaByStation&StationID="+StationID;//分界线后面是要传给下一界面的数据
//		alert("url="+url);
		xmlHttpGetAreaInfo.onreadystatechange=function(){GetAreaInfoRes();};//每当readyState值改变时便会触发函数
