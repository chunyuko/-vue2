...
function TreeVue() {
  ...
  $.post(
    '/api162/dalirobotcms/ajax/realTimeData.php',
    {
      ...
      GridDevType: 1, //1表计,2可见光,4红外,5声音
    },
    function (res) {
      res = eval('(' + res + ')')
      if (res.code == 200) {
...
function videoStop(event) {
  ...
  var deviceIDVal = document.getElementById('' + event.currentTarget.id + '').value
  $.post(
    '/api162/dalirobotcms/ajax/getCameraInfo.php',
    {
      task: 'getCameraByDevID',
