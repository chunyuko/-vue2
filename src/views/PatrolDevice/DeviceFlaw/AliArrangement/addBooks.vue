<template>
  <el-dialog :title="topTitle" class="addBooks" :visible.sync="dialogVisible" width="100%">
    <!-- 基本信息 -->
    <add-books-tags
      @father="closeDialog"
      :searchIs="addDatas"
...
export default {
  ...
  mounted() {
    ...
    _this.$bus.$on('dialogVisibledit', function (msg) {
      _this.dialogVisible = msg
    })
    _this.$bus.$on('maskIsShow', function (msg) {
      _this.dialogVisible = msg
    })
    _this.$bus.$on('bookAddTitle', function (msg) {
      _this.topTitle = msg
    })
    _this.$bus.$on('sheetEditTitle', function (msg) {
      _this.topTitle = msg
    })
  ...
  beforeDestroy() {
    ...
    _this.$bus.$off('bookAddTitle')
    _this.$bus.$off('shieetEditTitle')
  },
}
</script>
